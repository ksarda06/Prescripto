      ...
      <div>
        {
          ...
          <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b'key={index}>
            ...
            <div className='flex-1 text-sm text-zinc-600'>
              ...
              <p className='text-xs mt-1'><span className='text-sm text-neutrak-700 font-medium'>Date & Time:</span>{slotDateFormat(item.slotDate)}|{item.slotTime}</p>
            </div>
            <div></div>
            <div className='flex flex-col gap-2 justify-end'>
              ...
              {item.isCompleted && <button className='sm:min-w-48 py-2 border-red-500 rounded text-green-500'>Appointment completed</button>}
            </div>
          </div>
        ))}
      </div>
